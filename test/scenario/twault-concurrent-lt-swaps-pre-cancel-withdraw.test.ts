...
describe("LT Swap with withdraw then cancel after", async function () {
  ...
  describe ("Long-term swap withdraw checks", async function () {
  ...
  describe ("Long-term swap cancel part 2, withdraw partial proceeds", async function () {
    ...
    it ("should transfer the correct partial proceeds after cancelling the long-term order", async function() {
      ...
      expect(balChangeT1).to.be.closeTo(expectProceedsAddr1T1,
                                        tolerance,
                                        "LT Swap Customer Received Expected T1 Proceeds")
    })
  })
})