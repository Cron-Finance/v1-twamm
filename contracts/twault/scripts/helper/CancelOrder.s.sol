...
// forge script contracts/twault/scripts/helper/CancelOrder.s.sol:CancelOrderTestnetScript --rpc-url $GOERLI_RPC_URL --broadcast --slow -vvvv

// ensure correct orderId is passed in
contract CancelOrderTestnetScript is Script {
  function run() external {
    ...
    uint256 orderId = 0;
    // setup information for cancel order
    bytes memory userData = abi.encode(ICronV1PoolEnums.ExitType.Cancel, orderId);
    ...
    // send cancel order request to the pool via vault
