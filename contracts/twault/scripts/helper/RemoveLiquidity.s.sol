...
// forge script contracts/twault/scripts/helper/RemoveLiquidity.s.sol:RemoveLiquidityTestnetScript --rpc-url $GOERLI_RPC_URL --broadcast --slow -vvvv

contract RemoveLiquidityTestnetScript is Script {
  function run() external {
    ...
    uint256 numLPTokensToBurn = 100;
    // setup information for removing liquidity
    bytes memory userData = abi.encode(ICronV1PoolEnums.ExitType.Exit, numLPTokensToBurn);
    ...
    // send burn liquidity request to the pool via vault
